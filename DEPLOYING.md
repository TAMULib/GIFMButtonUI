...
```shell
docker image build -t gifmui .
docker run -it gifmui
```

<sub>_* Note: `-t gifmui` and `-it gifmui` may be changed to another tag name as desired, such as `-t developing_on_this` and `-it developing_on_this`._</sub><br>
